...
@section('header', app_term('edit') . ' ' . app_term('category') . ': ' . $category->name)
...
<div class="max-w-3xl mx-auto">
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-8 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <div>
                ...
                <p class="text-sm text-slate-500">{{ __('Modify the name and hierarchy of this classification.') }}</p>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="4" 
                          class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none"
                          placeholder="What is this category about?">{{ old('description', $category->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-4 py-2">
                        <label class="flex items-center cursor-pointer group">
                            <input type="radio" name="status" value="active" {{ old('status', $category->status) == 'active' ? 'checked' : '' }} class="hidden">
                        ...
                        <label class="flex items-center cursor-pointer group">
                            <input type="radio" name="status" value="inactive" {{ old('status', $category->status) == 'inactive' ? 'checked' : '' }} class="hidden">
                            <span class="w-5 h-5 border-2 border-slate-200 rounded-full flex items-center justify-center mr-2 group-hover:border-blue-300 transition-colors">
                                <span class="w-2 h-2 bg-blue-600 rounded-full scale-0 transition-transform duration-200"></span>
                            </span>
                ...
                <div class="space-y-2">
                    ...
                    @if($category->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/'.$category->image) }}" class="w-20 h-20 rounded-xl object-cover shadow-sm">
                        </div>
                    @endif
        ...
        <div class="pt-6 border-t border-slate-50 flex justify-end gap-4">
            ...
            <button type="submit" class="px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-100 transition-all flex items-center gap-2">
                ...
                {{ app_term('update') }} {{ app_term('category') }}
            </button>
        </div>
    </form>
</div>
